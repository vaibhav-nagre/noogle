...
export default function SearchBar({ query }) {
  const [search, setSearch] = useState("Search Noogle");
  const [showSearch, setShowSearch] = useState(false);
  const dropdownRef = useRef(null);
  const searches = [
    { search: "career rollercoaster", param: "nagre-projects" },
    { search: "education", param: "education" },
    { search: "why hire Nagre?", param: "why-hire-nagre" },
  ];
        ...
        <div className="flex items-center w-full px-4">
          ...
          <div
            className={`flex-grow px-4 py-2 bg-accent-color focus:outline-none ${
              search === "Search wsdfaf" ? "text-accent-text" : "text-white"
            }`}
            onClick={() => setShowSearch(!showSearch)}
          >
          ...
          <div className="flex flex-row justify-center items-center gap-x-2">
            ...
            <Link
              className="bg-no-repeat w-5 h-5 bg-cover"
              href={"https://calendar.app.google/ZnCHtsY49zCCrEYL6"}
              target="_blank"
